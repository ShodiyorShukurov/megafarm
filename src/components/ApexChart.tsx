...
const ApexChartData: React.FC<ChartProps> = ({ data }) => {
	...
	const [chartData, setChartData] = useState<{
		...
		series: [
			{ name: 'Jami summa', data: [] },
			{ name: 'Jami bonus', data: [] },
			{ name: 'Noyob foydalanuvchilar', data: [] },
		],
		options: {
			chart: {
				type: 'bar',
			...
			title: {
				text: 'Oylik tranzaksiya tendentsiyalari',
				align: 'left',
				style: {
					fontSize: '16px',
					fontWeight: 'bold',
			...
			tooltip: {
				...
				y: {
					formatter: (val: number, { seriesIndex }) => {
						...
						if (seriesIndex === 0) return `${val.toLocaleString()} UZS` // Jami summa
						if (seriesIndex === 1)
							return `${val.toLocaleString(undefined, { minimumFractionDigits: 2 })} UZS` // Jami bonus
						return `${val} foydalanuvchi` // Noyob foydalanuvchilar
					},
				},
			},
			legend: {
	...
	useEffect(() => {
		if (formattedData.length) {
			...
			setChartData(prevState => ({
				...prevState,
				series: [
					{ name: 'Jami summa', data: totalAmounts },
					{ name: 'Jami bonus', data: totalBonuses },
					{ name: 'Noyob foydalanuvchilar', data: uniqueUsers },
				],
				options: {
					...prevState.options,
					xaxis: {
	...
	if (!formattedData.length) {
		return (
			<div style={{ textAlign: 'center', padding: '20px', color: '#595959' }}>
				Diagramma uchun ma'lumot mavjud emas.
			</div>
		)
	}
